# Comprehensive Python environment management for causaliq-data
# Usage: .\scripts\setup-env.ps1 [options]

param(
    [switch]$Install,           # Install causaliq-data package after creating environments
...
function Show-Help {
    ...
    Write-Host "  .\scripts\setup-env.ps1 -Install           # Create + install causaliq-data" -ForegroundColor Gray
    Write-Host "  .\scripts\setup-env.ps1 -InstallOnly       # Install causaliq-data in existing envs" -ForegroundColor Gray
    Write-Host ""
...
if ($InstallOnly) {
    ...
    Write-Host "Installing causaliq-data in all environments..." -ForegroundColor Blue
    ...
    if ($Install) {
        Write-Host "(Will also install causaliq-data package)" -ForegroundColor Gray
    }
...
if (-not ($Install -or $InstallOnly)) {
    Write-Host "To install causaliq-data in all environments:" -ForegroundColor White
