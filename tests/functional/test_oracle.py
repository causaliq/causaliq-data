...
# Asia BN - set non-positive N
def test_set_N_value_error_4_ok():
    bn = read_bn(TESTDATA_DIR + "/discrete/small/asia.dsc")
    data = Oracle(bn=bn)
...
# Asia BN - no args
def test_set_order_type_error_1_ok():
    bn = read_bn(TESTDATA_DIR + "/discrete/small/asia.dsc")
    data = Oracle(bn=bn)
...
# Asia BN - bad arg type
def test_set_order_type_error_2_ok():
    bn = read_bn(TESTDATA_DIR + "/discrete/small/asia.dsc")
    data = Oracle(bn=bn)
...
# Asia BN - names mismatch
def test_set_order_value_error_1_ok():
    bn = read_bn(TESTDATA_DIR + "/discrete/small/asia.dsc")
    data = Oracle(bn=bn)
    nodes = bn.dag.nodes
...
# Asia BN - optimal/worst/original order
def test_set_order_asia_1_ok():
    bn = read_bn(TESTDATA_DIR + "/discrete/small/asia.dsc")
    data = Oracle(bn=bn)
